package com.example.wp.resource.widget.banner2.core;
...
import com.example.wp.resource.widget.banner2.callback.CreateViewCallBack;
...
public abstract class BaseBanner<T extends BaseBanner> extends FrameLayout {
	...
	private void dealAttrs(Context context, AttributeSet attrs) {
		...
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Banner2);
		interval = typedArray.getInt(R.styleable.Banner2_banner_interval, interval);
		isAutoPlay = typedArray.getBoolean(R.styleable.Banner2_banner_auto_play, isAutoPlay);
		isLoop = typedArray.getBoolean(R.styleable.Banner2_banner_loop, isLoop);
		isVertical = (typedArray.getInt(R.styleable.Banner2_banner_orientation, 0) == VERTICAL);
		typedArray.recycle();
	}
