...
import com.example.wp.resource.widget.banner.listener.OnBannerClickListener;
import com.example.wp.resource.widget.banner.listener.OnBannerListener;
import com.example.wp.resource.widget.banner.loader.ImageLoaderInterface;
import com.example.wp.resource.widget.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
	...
	private void handleTypedArray(Context context, AttributeSet attrs) {
		...
		mPageResId = typedArray.getResourceId(R.styleable.Banner_page_layout, -1);
			...
			if (view == null) {
				if (mPageResId == -1) {
					...
					if (imageContainer == null) {
						throw new IllegalArgumentException("please set tag 'imageContainer' to you imageContainer");
					}
					imageContainer.addView(imageViews.get(position));
				}
