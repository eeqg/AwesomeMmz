package com.example.wp.resource.widget.ninegrid;
...
public class NineGridView extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childrenCount; i++) {
			...
			LogUtils.d("-----onLayout()--mAdapter = " + mAdapter);
			if (mAdapter != null) {
	...
	private View getItemView(final int position) {
		...
		if (position < itemViews.size()) {
			itemView = itemViews.get(position);
		} else {
			itemView = mAdapter.onCreateView(getContext());
